...
{
	local X2CharacterTemplate CharTemplate;
	CharTemplate = Super(X2Character_DefaultCharacters).CreateTemplate_Chryssalid();
	CharTemplate.strPawnArchetypes.Remove(0, CharTemplate.strPawnArchetypes.Length);
	CharTemplate.strPawnArchetypes.AddItem("GW_GameUnits.ARC_GameUnit_ChryssalidEdited");
	return CharTemplate;
}
...
{
    local X2CharacterTemplate CharTemplate;
    CharTemplate = Super(X2Character_DefaultCharacters).CreateTemplate_PsiZombie(TemplateName);
    CharTemplate.strPawnArchetypes.Remove(0, CharTemplate.strPawnArchetypes.Length);
    CharTemplate.strPawnArchetypes.AddItem("GW_GameUnits.ARC_GameUnit_ZombieEdited_M");
    CharTemplate.strPawnArchetypes.AddItem("GW_GameUnits.ARC_GameUnit_ZombieEdited_F");
	return CharTemplate;
}


//Codex(Cyberus) does not require it (brain collected)
//Chrysallid cannot map to Body anim set but we can still give itsome SWEET RAGDOLL PHYSICS WOOOOOOOOOOO
